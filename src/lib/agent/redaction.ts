...
/** Deterministic findings before provider redaction; never returns sensitive values. */
export function detectLocalPiiTypes(text: string): string[] {
  const types: string[] = [];
  if (new RegExp(NIK_PATTERN.source).test(text)) types.push("national_id");
  if (new RegExp(PHONE_PATTERN.source).test(text.replace(NIK_PATTERN, "")))
    types.push("phone");
  if (new RegExp(EMAIL_PATTERN.source).test(text)) types.push("email");
  return types;
}

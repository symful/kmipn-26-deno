...
>;
export type AnalyzeImageVisionOutput = z.infer<
  typeof analyzeImageVisionOutputSchema
>;
...
const analyzeImageVisionTool = {
  ...
  execute: async (
    env: Env,
    input: AnalyzeImageVisionInput,
  ): Promise<AnalyzeImageVisionOutput> => {
    try {
      const result = await assessWithVision(env, input.image_source, {

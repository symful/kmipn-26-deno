...
import { detectLocalPiiTypes } from "@/lib/agent/redaction";
...
export type PrivacyRiskVisionInput = z.infer<
  typeof privacyRiskVisionInputSchema
>;

export const privacyRiskTextDescriptor: ToolDescriptor = {
  name: "detect_privacy_risk_text",
  model: "text",
  inputSchema: privacyRiskTextInputSchema,
  promptBuilder: (input: unknown): string => {
    const typedInput = input as PrivacyRiskTextInput;
    return `Analisis risiko privasi TEKS SAJA dari deskripsi laporan berikut.
Foto diperiksa secara terpisah oleh pemeriksa visual. Jangan menilai foto atau mencatat ketiadaan lampiran sebagai risiko maupun ketidakpastian dalam pemeriksaan teks ini.
Penanda redaksi seperti [NAME_REDACTED], [PHONE_REDACTED], atau [REDACTED] bukan PII yang masih terbuka dan bukan alasan ketidakpastian. Tetap periksa teks lain di sekitarnya untuk data pribadi yang benar-benar terlihat.
Jangan menebak identitas di balik penanda redaksi. Isi risk_factors hanya dengan risiko privasi yang didukung teks yang tersedia.
...
export const privacyRiskVisionDescriptor: ToolDescriptor = {
  ...
  model: "vision",
  inputSchema: privacyRiskVisionInputSchema,
  promptBuilder: (input: unknown): string => {
    const typedInput = input as PrivacyRiskVisionInput;
    return `Analisis foto-foto berikut untuk risiko privasi berdasarkan bukti yang benar-benar terlihat.
Jangan menebak nama, identitas penghuni, alamat pribadi, atau informasi pribadi hanya dari keberadaan bangunan, jalan, atau fasilitas umum. Laporkan wajah, pelat, tulisan alamat pribadi, atau dokumen hanya jika terlihat pada gambar.
Jangan menyalin data pribadi yang terbaca ke hasil; sebutkan jenis dan lokasi bagian gambar yang perlu disamarkan. Keterbatasan visual hanya boleh disebut bila didukung kualitas gambar yang benar-benar diperiksa.
...
const privacyRiskTool = {
  ...
  description:
    "Detects privacy risks in report description and images using both text and vision models. Combines results for comprehensive assessment.",
  ...
  ): Promise<PrivacyRiskOutput> => {
    try {
      const textResult = await callLLM(env, {
        tool: privacyRiskTextDescriptor,
      ...
      const visionResults = await Promise.all(
        input.photo_urls.map((photo_url) =>
          callLLM(env, {
            tool: privacyRiskVisionDescriptor,
            input: { report_id: input.report_id, photo_urls: [photo_url] },
          }),
        ),
      );

      const allVisionData = visionResults as PrivacyRiskOutput[];
      const localPiiTypes = detectLocalPiiTypes(input.description);
      let pii_detected = textData.pii_detected || localPiiTypes.length > 0;
      let redaction_needed =
        textData.redaction_needed || localPiiTypes.length > 0;
      ...
      const pii_types = [
        ...new Set([...localPiiTypes, ...textData.pii_types, ...allPiiTypes]),
      ];
      const supporting_factors = [
        ...new Set([...textData.supporting_factors, ...allSupportingFactors]),
      ];
      const risk_factors = [
        ...new Set([
          ...localPiiTypes.map((type) => `local_text_pii:${type}`),
          ...textData.risk_factors,
          ...allRiskFactors,
        ]),
      ];

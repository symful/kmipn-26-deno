...
function getAllowedHosts(env: Env): string[] {
  ...
  return (
    ...
    "r2.cloudflarestorage.com,sigap.live,localhost"
  ).split(",");
}
...
export async function assessWithVision(
  env: Env,
  imageUrl: string,
  reportContext: { description: string; category_name: string },
  ...
  try {
    ...
    if (
      ...
      return {
        ...
        confidence: 0,
        description: "",
        vlm_error: "host_not_allowed: " + u.hostname,
      };
    }
    ...
    const res = await sendProviderRequest(env, {

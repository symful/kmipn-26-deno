...
export const adminDaerahOperatorsRoute = new Hono<{ Bindings: Env; Variables: AuthVariables }>();
...
adminDaerahOperatorsRoute.get(
  "/",
  requireAuth,
  requireRole("ADMIN_DAERAH", "ADMIN"),
  safeHandler(async (c) => {
    const user = c.get("user");
    const query = ListOperatorsQuerySchema.safeParse(c.req.query());
    if (!query.success) {
    ...
    const filters: string[] = ["role = 'OPERATOR'", "deleted_at IS NULL"];

...
healthRoute.get(
  ...
  safeHandler(async (c) => {
    let db = false;
    try {
      await c.env.D1.prepare("SELECT 1").bind().all();
      db = true;
    } catch (e) {
      logger.warn({
        route: "/health",
        method: "GET",
        context: "db_health_check_failed",
    ...
    try {
      ...
      logger.warn({
        route: "/health",
        method: "GET",
        context: "llm_health_check_failed",
        error: e instanceof Error ? e : new Error(String(e)),
      });
    }
    let r2 = false;
    try {
      ...
      logger.warn({
        route: "/health",
        method: "GET",
        context: "r2_health_check_failed",
        error: e instanceof Error ? e : new Error(String(e)),
      });
    }
    let kv = false;
    if (c.env.RATE_LIMITER) {
      try {
        ...
        logger.warn({
          route: "/health",
          method: "GET",
          context: "kv_health_check_failed",
          error: e instanceof Error ? e : new Error(String(e)),
        });
      }
    }
    return c.json({
      status: db && llm && r2 && kv ? "ok" : "degraded",
      db,
      llm,
      r2,
      kv,

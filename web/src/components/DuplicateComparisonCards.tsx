  ...
  return (
    ...
    >
      ...
      <div className="space-y-2 text-sm">
        ...
        <div
          className={`${commonAttributes?.has("category") ? "bg-primary-50 px-2 py-1 rounded" : ""}`}
        >
          <span className="text-xs text-gray-500">Kategori:</span>
          <span className="ml-1 text-gray-700">
            {report.category?.name ?? "Kategori belum tersedia"}
          </span>
        </div>

        <div
          className={`${commonAttributes?.has("location") ? "bg-primary-50 px-2 py-1 rounded" : ""}`}
        >
          <span className="text-xs text-gray-500">Lokasi:</span>
          <span className="ml-1 text-gray-700 font-mono text-xs">
            {location}
          </span>
        </div>

        <div
          className={`${commonAttributes?.has("created_at") ? "bg-primary-50 px-2 py-1 rounded" : ""}`}
        >
        ...
        <div
          className={`${commonAttributes?.has("severity") ? "bg-primary-50 px-2 py-1 rounded" : ""}`}
        >
          <span className="text-xs text-gray-500">Tingkat kerusakan:</span>
          <span className="ml-1 text-gray-700">
            {report.severity != null ? `${report.severity}%` : "-"}
          </span>
        </div>
      </div>
    </div>
...
export const DuplicateComparisonCards = ({
  ...
  const [candidateReports, setCandidateReports] = useState<
    Record<string, ReportWithDistance>
  >({});
  ...
  useEffect(() => {
    if ((duplicateCandidates?.length ?? 0) === 0) return;
    ...
    const fetchCandidates = async () => {
      ...
      try {
        await Promise.all(
          duplicateCandidates.map(async (candidate) => {
            try {
              ...
              logger.error(`Failed to fetch report ${candidate.report_id}`, {
                error: e,
              });
            }
          }),
        ...
        setError(
          e instanceof Error ? e.message : "Failed to fetch candidate reports",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleMerge = async (targetReportId: string) => {
    ...
    try {
      await api.combineCases(currentReport.id, {
  ...
  const handleKeepSeparate = async (reportId: string) => {
    ...
    try {
      await api.separateCase(reportId, {
        new_case_description: `Laporan duplikat - ${currentReport.category?.name ?? "kategori belum tersedia"}`,
  ...
  const findCommonAttributes = (candidate: Report): Set<string> => {
    ...
    if (candidate.category_id === currentReport.category_id)
      common.add("category");
    if (
      candidate.lat === currentReport.lat &&
      candidate.lng === currentReport.lng
    )
      common.add("location");
    if (candidate.created_at === currentReport.created_at)
      common.add("created_at");
    if (candidate.severity === currentReport.severity) common.add("severity");
    return common;
  };

  if ((duplicateCandidates?.length ?? 0) === 0) {
    return null;
  }
      ...
      <div className="px-4 py-3 border-b border-sigap-border bg-gray-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            />
            <h3 className="text-sm font-semibold text-sigap-textPrimary">
              Laporan untuk dibandingkan ({duplicateCandidates?.length ?? 0})
            </h3>
          </div>
          <span className="text-xs text-sigap-textMuted">
            Warna hijau menandai nilai yang sama. Periksa foto dan uraian untuk
            memastikan kedua laporan membahas kejadian yang sama.
          </span>
        </div>
      </div>

...
export const CreateReport = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let deviceId = localStorage.getItem("sigap_device_id");
    if (!deviceId) {
      deviceId = crypto.randomUUID();
      localStorage.setItem("sigap_device_id", deviceId);
    }

    try {
      // Upload photos anonymously FIRST so their public URLs can be attached
      // to the report at creation time. The public create-report endpoint is
      // unauthenticated, so we cannot rely on the authenticated photo endpoint
      // (auto-attach) — we must upload via the anon endpoint and pass the
      // returned URLs in the create body.
      ...
      const result = await api.createReportPublic({
  ...
  return (
    <div className="min-h-screen bg-sigap-background overflow-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-sigap-textMuted">
                Pelaporan Infrastruktur &amp; Fasilitas Desa
              </p>
            </div>
          </div>
          <Link
      ...
      <main className="p-6 max-w-3xl mx-auto">
        <div className="bg-white rounded-xl border border-sigap-border p-6 shadow-sm">
          ...
          {success && (
            <div
              ...
              style={{
                backgroundColor: colors.selesai + "20",
                borderColor: colors.selesai + "40",
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {(photos?.length ?? 0) > 0 && (
                  <div className="mt-3 grid grid-cols-5 gap-2">
                    {photos.map((photo, index) => (
                      <div
                        key={index}
                        className="relative aspect-square rounded-lg overflow-hidden border border-sigap-border"
                      >
                        <img
                          src={photo.preview}
                          alt={`Preview ${index + 1}`}
                        ...
                        {photo.exifGps && (
                          <div
                            ...
                            style={{ backgroundColor: colors.selesai + "CC" }}
                          >
                            GPS
                          </div>
                        )}
                ...
                <p className="text-xs text-sigap-textMuted mt-1">
                  Tambahkan hingga 5 foto JPG atau PNG yang memperlihatkan
                  kondisi fasilitas. Periksa titik lokasi laporan sebelum
                  mengirim.
                </p>
                {exifFailed && (
                  <p className="text-xs mt-1" style={{ color: colors.warning }}>
                    Foto ini tidak menyediakan informasi lokasi atau waktu yang
                    dapat dibaca. Tentukan lokasi laporan di bawah; titik yang
                    Anda pilih tidak mengubah informasi asli pada foto.
                  </p>
                )}
              </div>
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  {gps.permissionDenied && (
                    <span className="text-xs text-warning-600">
                      GPS tidak diizinkan. Silakan aktifkan izin lokasi di
                      pengaturan browser.
                    </span>
                  )}
                </div>

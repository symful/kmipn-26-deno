...
export const WargaCreateReport = () => {
  ...
  useEffect(() => {
    api
      ...
      .catch((e) => { logger.error("Failed to fetch categories", { error: e }); setCategories([]); })
  ...
  const uploadPhoto = async (reportId: string, photo: PhotoFile): Promise<string> => {
    const contentType = photo.file.type === "image/png" ? "image/png" : "image/jpeg";
  ...
  const rollbackPhotos = async (uploadedUrls: string[]): Promise<void> => {
    for (const url of uploadedUrls) {
      try {
        await request(`/reports/photos/rollback`, {
          method: "POST",
          body: JSON.stringify({ photo_url: url }),
          token: true,
        });
      } catch (err) {
        logger.error("Rollback failed: " + (err instanceof Error ? err.message : String(err)), { error: err });
        // best effort — user already notified of main error
      }
    }
  };
      ...
      if (photos.length > 0) {
        ...
        try {
          for (const photo of photos) {
            const url = await uploadPhoto(result.id, photo);
            uploadedUrls.push(url);
          }
        } catch (uploadErr) {
          await rollbackPhotos(uploadedUrls);
          setError(uploadErr instanceof Error ? uploadErr.message : "Gagal mengunggah foto. Laporan tetap tersimpan.");
          ...
          {loading ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                style={{ backgroundColor: colors.primary }}
              >
                {uploadingPhotos ? "Mengunggah foto..." : submitting ? "Mengirim..." : "Kirim Laporan"}
              </button>
            </form>
          )}
        </div>
